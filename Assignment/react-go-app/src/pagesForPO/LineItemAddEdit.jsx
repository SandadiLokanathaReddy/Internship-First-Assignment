...
export const LineItemAddEdit = () => {
	const location = useLocation();
	const navigate = useNavigate();
	const { ponumber, lineItem, add, edit } = location.state;
	const heading = add === true ? "Add Line Item" : "Update Line Item";
	...
	const updatePOLineItemDetails = async (poli) => {
		const PUT_URL = "http://localhost:5050/updatepolineitem";
		await axios
			.put(PUT_URL, poli)
			.then((response) => {
				const params = {
	...
	const handleSubmit = (e) => {
		e.preventDefault();
		if (edit === true) {
			updatePOLineItemDetails(lineItemDetails);
		} else if (add === true) {
					...
					<div className="labels-container">
						...
						<label className="label">
							...
							<input
								...
								type="text"
								pattern="[A-Za-z ].{2,}"
								title="Provide a valid color name"
